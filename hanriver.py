...
# 기본 입수 지점.
#
# 마포대교 중심(OSM 실측 37.53345, 126.93626)과 사실상 같은 지점이다.
# 원래는 배경 사진의 다리 픽셀에서 역산한 값이었는데, 실제 다리 위였다.
#
...
# ─────────────────────────────────────────
# [3-1] 실물 축소 지도 영역 (3m × 2m, 축척 1:600)
# ─────────────────────────────────────────
M_PER_DEG_LON = 88000    # 위도 37.5° 기준
M_PER_DEG_LAT = 111000

# 인쇄물은 3:2 여야 한다. 화면(네이버 지도)과 바닥 인쇄물의 비율이 다르면
# 지형이 늘어나거나 눌려서 좌표가 그만큼 어긋난다.
MAP_SCALE   = 600     # 1:600 — 덮는 실제 1800 x 1200 m
MAP_PRINT_W = 3.0     # 실물 지도 가로 m (동서)
MAP_PRINT_H = 2.0     # 실물 지도 세로 m (남북)
# 입수 지점에서 동/남쪽 여유. 실물 지도 사각형이 이 값으로 자리잡는다.
#
# 예전에는 배경 사진(mapomap.jpg)의 픽셀에 별표가 얹히도록 역산한 값이었다.
# 지금은 배경이 네이버 지도(실제 위경도)라 사진에 묶일 이유가 없어졌고,
# 대신 OSM 실측 지형을 담도록 잡았다:
#     밤섬            경도 126.9220 ~126.9345  위도 37.5358 ~37.5413
#     마포대교        경도 126.92972~126.94280  위도 37.52931~37.53760
#     S-OIL(용강주유소)                126.93871              37.54301
# 입수 지점(37.5336, 126.9364)은 마포대교 중심(37.53345, 126.93626)과 일치한다.
#
# 결과 bbox: 126.92097,37.53246 ~ 126.94143,37.54328
#   밤섬 전체 + 입수 지점 + S-OIL 을 모두 담는다.
#   팀이 지정한 참고 화면(밤섬 픽셀로 역산: 126.92084,37.53268 ~ 126.94156,37.54306)
#   과 각 변이 25m 이내로 일치한다.
# 인쇄물은 이 bbox 로 뽑아야 바닥과 화면이 맞는다.
MAP_EAST_M  = 442.5    # 입수 지점에서 동쪽 여유 (나머지는 서쪽 표류 구간)
MAP_SOUTH_M = 126.0    # 입수 지점에서 남쪽 여유 (나머지는 북쪽 구간)
