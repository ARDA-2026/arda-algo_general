...
| 비행 경로 | 초록 실선 / 흰 파선 | 완료한 레그 / 예정 레그 |
| 건너뛴 레그 | 회색 점선 + ✕ | 이동량 20cm 미만이라 실행 안 함 |
...
| **발진 차단 시** | 버튼 아래에 사유가 문장으로 표시됨 (`배터리 부족 (24% / 최소 30%)` 등) |

발진 조건은 [tello_driver.py](tello_driver.py) 의 검사를 그대로 미러링합니다 — 연결됨 ∧ 미실행 ∧ 모든 waypoint 지도 안 ∧ 배터리 ≥30% ∧ 모든 레그 ≤500cm.
